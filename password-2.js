...
if (stringPassword.length < 8) { //make sure length is 8 or greater
    stringOutput = "Password is too short-try again";
}  else {
        for (i = 0; i < arrayBadPasswords.length; i++) { //looping to check all 500 of the bad passwords in the array and if they are equal to the stringPassword
            if (stringPassword == arrayBadPasswords[i]) {  //comparing the entered password with the values in the array, one at a time
                stringOutput = "Password matches list of bad passwords-try again";
            } 
        } if (stringOutput == ""){ //if the stringOutput is blank(it passed both previous tests), then it sets the new stringOutput
            stringOutput = "Great password!";
        }
}
   $("output").value = stringOutput; //sending the stringOutput back to the DOM to be displayed
};
